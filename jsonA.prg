	...
	*
	...
	local cTipo
		* Cuando se manda una arreglo, 
		...
		case cTipo=='O'
			local cProp, cJsonValue, cRetVal, aProp[1], cPropName, cPropConDatos
			=AMembers(aProp,xExpr)
			cRetVal = ''
			for each cProp in aProp
				*?? cProp,','
				*? cRetVal
				IF ( m.cProp != '__' AND PROPER( m.cProp) != 'Array')		&& solo revisa los 2 guiones bajos(nombre correcto)	11/11/2022 Azzor Delinius
					LOOP
				ENDIF
				IF PROPER( m.cProp) = 'Array'
					m.cPropName = ''
					m.cPropConDatos = m.cProp
					
				ELSE
					m.cPropName = EVALUATE( 'xExpr.' + m.cProp)								&& 11/11/2022 Azzor Delinius
					m.cPropConDatos = SUBSTR( m.cProp, 2)								&& 11/11/2022 Azzor Delinius
				ENDIF
													
				
				if type('xExpr.'+ m.cPropConDatos)=='U' or m.cPropConDatos=='CLASS'
					* algunas propiedades pueden no estar definidas
					* como: activecontrol, parent, etc
					loop
				ENDIF
*!*					m.cPropiedad = 'xExpr..' + m.cProp
*!*	*				&cPropiedad.Comment = m.cProp			&& 11/11/2022 para corregir las mayusculas en el nombre de las propiedades	Azzor Delinius
*!*					borrar esto!



				IF TYPE( 'ALen(xExpr.'+ m.cPropConDatos+')' ) == 'N'
					...
					*
					Local i,nTotElem
					cJsonValue = ''
					nTotElem = EVALUATE( 'ALen(xExpr.'+ m.cPropConDatos + ')')
					For i=1 to nTotElem
...
*!*							&cmd.
						...
						m.uborrame = EVALUATE( 'xExpr.' + m.cPropConDatos + '[i]')
						m.cJsonValue = m.cJsonValue + "," + this.encode( EVALUATE( 'xExpr.' +m.cPropConDatos + '[i]'))		&& 11/11/2022 mas facil de entender Azzor Delinius
					Next
					cJsonValue = '[' + substr(cJsonValue,2) + ']'
					IF m.cJsonValue == '[false]'
						m.cJsonValue = '[]'
					ENDIF
				else
					*
					...
					*
					cJsonValue = this.encode( evaluate( 'xExpr.'+cPropConDatos) )				
				endif
*!*					if left(cProp,1)=='_'
*!*						m.cPropName = substr( m.cPropName, 3)		&& ANTES substr( m.cProp,3)		&& 11/11/2022 Azzor Delinius
*!*					endif
				IF EMPTY( m.cPropName)								&& Arrays 11/11/2022	Azzor Delinius
					cRetVal = cRetVal + cJsonValue			&& Arrays 11/11/2022	Azzor Delinius
				ELSE
					cRetVal = cRetVal + ',' + '"' + m.cPropName + '":' + cJsonValue		&& quitado LOWER	11/11/2022	Azzor Delinius
				ENDIF
			NEXT
			IF EMPTY( m.cPropName)				&& Arrays 11/11/2022	Azzor Delinius
				RETURN m.cRetVal				&& Array 11/11/2022	Azzor Delinius
			ENDIF
...
Hidden ;
	...
	function set(cPropName, xValue)
		this.addProperty('__' + m.cPropName, m.cPropName)	&& Doble guion bajo para poner el nombre exacto de la propiedad(mayus y minus)	11/11/22	Azzor Delinius
		
		m.cPropName = '_' + m.cPropName
		
		DO CASE
		case type('ALen(xValue)')=='N'
			* es un arreglo
			local nLen,cmd,i
			this.addProperty( m.cPropName+'(1)')
			nLen = alen(xValue)
...
*!*				&cmd
				this.&cPropName = xValue	&& mas entendible asi... 11/11/2022	Azzor Delinius
		endcase
	return
...
*!*				&cmd
			return EVALUATE( 'this.&' + m.cPropName)		&& mas entendible asi... 11/11/2022	Azzor Delinius
		endif
	return ''
